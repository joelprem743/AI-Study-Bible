      ...
      const [isPausedUI, setIsPausedUI] = useState(false);
        ...
        const { play, stop, preload, pause, resume, subscribeProgress } = useAudio();
        ...
        const handleNextVerse = () => {
          const next = Math.min(currentVerseIndex + 1, verses.length - 1);
        
          playbackIdRef.current++;
          playVerseSequence(next);
        };
        
        const handlePrevVerse = () => {
          const prev = Math.max(currentVerseIndex - 1, 0);
        
          playbackIdRef.current++;
          playVerseSequence(prev);
        };
        ...
        }, [selectedBook, selectedChapter, continuousPlay, stop]);
      ...
      const playVerseSequence = useCallback(async (startIndex = 0) => {
        if (!verses.length) {
          setIsPlaying(false);
        ...
        for (let i = startIndex; i < verses.length; i++) {
          ...
          const text =
            verse.text[singleVersion] ??
            Object.values(verse.text).find(t => t && t.trim());
          ...
          try {
            ...
            if (nextVerse) {
              const nextText =
                nextVerse.text[singleVersion] ??
                Object.values(nextVerse.text).find(t => t && t.trim());
      
              if (nextText) {
                preload({
                  text: nextText,
                  lang: singleVersion === "TELUGU_COMMUNITY_V1" ? "TE" : "EN",
                });
              }
            }
  ...
  words < 8
    ? 900
    : Math.max(700, Math.min(1600, words * 40));
            
            await new Promise((r) => setTimeout(r, gapMs));
            
            // 🔥 INTERRUPT CHECK AFTER GAP
            if (id !== playbackIdRef.current) return;
        ...
        if (continuousPlay && id === playbackIdRef.current) {
          handleNextChapter();
          setCurrentVerseIndex(0);
          return; // 🔥 STOP HERE — do NOT trigger playback yet
        } else {
          setIsPlaying(false);
          isPlayingRef.current = false;
        }
      ...
      const togglePlay = useCallback(() => {
        if (isPlaying) {
          playbackIdRef.current++; // 🔥 kill loop
          stop();
      ...
      }, [isPlaying, currentVerseIndex, playVerseSequence, stop]);


      const handleStop = () => {
        // 🔥 kill running playback loop
        playbackIdRef.current++;
        ...
        useEffect(() => {
          const unsub = subscribeProgress((p, d) => {
            setAudioProgress(p);
            setAudioDuration(d);
          });
        ...
        const verseText =
          currentVerse?.text?.[singleVersion] ??
          Object.values(currentVerse?.text || {}).find(t => t && t.trim());
                    ...
                    <>
                      ...
                      <div className="w-full md:w-2/3 flex flex-col flex-1 min-h-0">
                        ...
                        <NavigationPane
                          ...
                          onToggleAudioBar={() => setShowAudioBar(v => !v)}
    ...
    ">
      ...
      {/* 🔥 NOW PLAYING (Verse indicator) */}

      <div className="w-full max-w-md text-center mb-2">

      <div className="text-[11px] text-slate-500 dark:text-slate-400">
        Playing {currentPosition} of {totalVerses} verses
      </div>

      <div className="text-xs font-medium text-slate-700 dark:text-slate-200 truncate">
        {selectedBook} {selectedChapter}:{currentVerse?.verse || 1}
        {verseText && <> — {verseText.slice(0, 35)}...</>}
      </div>
      ...
      <div className="flex items-center justify-center gap-4">
        ...
        <button
          ...
          "
        >
          <i className={`fas ${isPlaying && !isPausedUI ? "fa-pause" : "fa-play"} text-lg`} />
        </button>
